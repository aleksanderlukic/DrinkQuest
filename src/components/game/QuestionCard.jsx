...
const TYPE_CONFIG = {
  truth: { label: 'Truth', emoji: '💬', gradient: 'from-violet-600 to-purple-700' },
  dare: { label: 'Dare', emoji: '🎯', gradient: 'from-pink-600 to-rose-700' },
  statement: { label: 'Never Have I Ever', emoji: '🙅', gradient: 'from-pink-500 to-rose-600' },
  prompt: { label: 'Vote', emoji: '☝️', gradient: 'from-amber-500 to-orange-600' },
  question: { label: 'Question', emoji: '❓', gradient: 'from-blue-500 to-cyan-600' },
  action: { label: 'Action', emoji: '⚡', gradient: 'from-emerald-500 to-teal-600' },
};
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <div className={`bg-gradient-to-br ${cfg.gradient} p-6 text-center`}>
          ...
          <div className="mt-2 text-white/80 text-xs font-bold uppercase tracking-widest">{cfg.label}</div>
          {player && (
        ...
        <div className="p-8 text-center">
          ...
          {question.difficulty && (
            <div className={`mt-6 inline-block px-3 py-1 rounded-full text-xs font-semibold ${
              ...
              {question.difficulty === 'soft' ? '🌱' : question.difficulty === 'normal' ? '🔥' : '💀'} {question.difficulty}
            </div>
          )}
        </div>
      </motion.div>
