...
export default function SpinTheBottlePage() {
  ...
  const modeLabels = {
    random: { label: t('spin.modeRandom'), emoji: '🎲' },
    truth: { label: t('spin.modeTruth'), emoji: '💬' },
    dare: { label: t('spin.modeDare'), emoji: '🎯' },
    drink: { label: t('spin.modeDrink'), emoji: '🥃' },
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className={`rounded-3xl p-6 space-y-6 mb-6 ${isDark ? 'bg-slate-900 border border-white/10' : 'bg-white border border-slate-200 shadow-lg'}`}>
          {/* Players */}
          <div>
          ...
          {/* Spin mode */}
          <div>
            <h3 className={`font-bold text-sm mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('spin.spinMode')}</h3>
        ...
        {/* Spin component */}
        <div className={`rounded-3xl p-8 ${isDark ? 'bg-slate-900 border border-white/10' : 'bg-white border border-slate-200 shadow-lg'}`}>
