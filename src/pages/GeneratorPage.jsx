...
export default function GeneratorPage() {
  ...
  const toneLabels = {
    funny: { label: t("generator.tones.funny") },
    wild: { label: t("generator.tones.wild") },
    awkward: { label: t("generator.tones.awkward") },
    flirty: { label: t("generator.tones.flirty") },
    brutal: { label: t("generator.tones.brutal") },
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Form */}
        <div
          className={`rounded-3xl p-6 space-y-5 mb-6 ${isDark ? "bg-slate-900 border border-white/10" : "bg-white border border-slate-200 shadow-lg"}`}
        >
          <div className="grid grid-cols-2 gap-4">
            {["event", "place", "vibe", "group"].map((field) => (
              <div key={field}>
                ...
                >
                  {t(`generator.form.${field}`)}
                </label>
                <input
                  value={form[field]}
                  onChange={(e) => handleChange(field, e.target.value)}
                  placeholder={t(`generator.form.${field}Placeholder`)}
          ...
          <div>
            <label
              className={`block text-xs font-semibold mb-2 ${isDark ? "text-slate-400" : "text-slate-500"}`}
            >
          ...
          <div className="flex items-center gap-4">
            <label
              className={`text-xs font-semibold whitespace-nowrap ${isDark ? "text-slate-400" : "text-slate-500"}`}
            >
              {t("generator.form.generate")}: {form.count}
            </label>
            <input
              type="range"
              min="5"
        ...
        {results.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              >
                {results.length} {t("generator.results.title")}
              </p>
              <div className="flex gap-2">
                ...
                >
                  {t("generator.results.saveSelected")} ({selected.size})
                </button>
                <button
                ...
                >
                  {saved
                    ? t("generator.results.saved")
                    : t("generator.results.saveAll")}
                </button>
              </div>
            </div>
