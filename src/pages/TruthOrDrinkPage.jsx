...
export default function TruthOrDrinkPage() {
  const { t, isDark, language, players, addPlayer, removePlayer, settings, customTod } = useApp();
      ...
      <div className="min-h-screen pt-24 pb-16 px-4">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-8">
            ...
            <h1 className={`text-3xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('games.truthOrDrink.title')}</h1>
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('games.truthOrDrink.description')}</p>
          </div>
          <div className={`rounded-3xl p-6 space-y-6 ${isDark ? 'bg-slate-900 border border-white/10' : 'bg-white border border-slate-200 shadow-lg'}`}>
            <div>
              <h3 className={`font-bold text-sm mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('game.setup.players')}</h3>
            ...
            <div className={`p-4 rounded-xl text-sm ${isDark ? 'bg-amber-900/20 border border-amber-500/20 text-amber-300' : 'bg-amber-50 border border-amber-200 text-amber-700'}`}>
              {t('games.truthOrDrink.rule')}
            </div>
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-xl mx-auto space-y-6">
        <div className="text-center">
          <h1 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('games.truthOrDrink.title')}</h1>
          <p className={`text-xs mt-1 ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>{t('games.truthOrDrink.ruleShort')}</p>
        </div>
        {allUsed ? (
