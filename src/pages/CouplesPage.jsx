...
export default function CouplesPage() {
  const { t, isDark, language, players, addPlayer, removePlayer, settings, customCouples } = useApp();
      ...
      <div className="min-h-screen pt-24 pb-16 px-4">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-8">
            ...
            <h1 className={`text-3xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('games.couples.title')}</h1>
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('games.couples.description')}</p>
          </div>
          <div className={`rounded-3xl p-6 space-y-6 ${isDark ? 'bg-slate-900 border border-white/10' : 'bg-white border border-slate-200 shadow-lg'}`}>
            <div>
              <h3 className={`font-bold text-sm mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                {t('game.setup.players')} <span className={`font-normal text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>({t('games.couples.twoPlayers')})</span>
              </h3>
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-xl mx-auto space-y-6">
        <div className="text-center">
          <h1 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('games.couples.title')}</h1>
        </div>
        {allUsed ? (
