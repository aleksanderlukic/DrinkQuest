...
export default function MostLikelyToPage() {
  const { t, isDark, language, players, addPlayer, removePlayer, settings, customMlt } = useApp();
      ...
      <div className="min-h-screen pt-24 pb-16 px-4">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-8">
            ...
            <h1 className={`text-3xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('games.mostLikelyTo.title')}</h1>
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('games.mostLikelyTo.description')}</p>
          </div>
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-xl mx-auto space-y-6">
        <div className="text-center">
          <h1 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('games.mostLikelyTo.title')}</h1>
          <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('games.mostLikelyTo.instruction')}</p>
        </div>
        {allUsed ? (
