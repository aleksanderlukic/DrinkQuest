...
export default function CustomPage() {
  const { t, isDark, customTruths, setCustomTruths, customDares, setCustomDares, customNhie, setCustomNhie, customMlt, setCustomMlt } = useApp();
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className={`text-3xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('custom.title')}</h1>
          <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('custom.subtitle')}</p>
        </div>
        ...
        {/* Add input */}
        <div className={`rounded-2xl p-5 mb-5 ${isDark ? 'bg-slate-900 border border-white/10' : 'bg-white border border-slate-200 shadow-sm'}`}>
          <h3 className={`font-bold text-sm mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('custom.addNew')}</h3>
        ...
        {/* List */}
        {list.length === 0 ? (
          <div className={`text-center py-12 rounded-2xl ${isDark ? 'bg-slate-900 border border-white/5' : 'bg-slate-50 border border-slate-200'}`}>
            <p className={`text-3xl mb-3`}>📝</p>
            <p className={`text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{t('custom.empty')}</p>
          </div>
        ) : (
          <div className="space-y-3">
            <p className={`text-xs font-semibold ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
              {list.length} {t('custom.items')}
            </p>
            {list.map((item) => (
              <div
                key={item.id}
              ...
              >
                <p className={`flex-1 text-sm leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                  {item.text}
                </p>
                <button
